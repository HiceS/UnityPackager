...
class Material:
    ...
    def __init__(
        ...
        Args:
            ...
            matType (int, optional): rendering type Enum.
        """
    ...
    def serialize(self) -> str:
        ...
        if self.matType == MatType.STANDARD.value:
            return GenerateYamlData(material_data, matBasicYaml)
        elif self.matType == MatType.URP.value:
            return GenerateYamlData(material_data, urp_mat)
        else:
            raise RuntimeError(
                f"Specified Renderer Type is not support currently {self.matType}"
            )
